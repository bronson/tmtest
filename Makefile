...
status1.o: status1.re status.h r2scan.h Makefile
	re2c $(REOPTS) status1.re > status1.c
	perl -pi -e 's/^\#line.*$$//' status1.c
	$(CC) -g -c status1.c

status2.o: status2.re status.h r2scan.h Makefile
	re2c $(REOPTS) status2.re > status2.c
	perl -pi -e 's/^\#line.*$$//' status2.c
	$(CC) -g -c status2.c

status3.o: status3.re status.h r2scan.h Makefile
	re2c $(REOPTS) status3.re > status3.c
	perl -pi -e 's/^\#line.*$$//' status3.c
	$(CC) -g -c status3.c

status4.o: status4.re status.h r2scan.h Makefile
	re2c $(REOPTS) status4.re > status4.c
	perl -pi -e 's/^\#line.*$$//' status4.c
	$(CC) -g -c status4.c

tfscan.o: tfscan.re tfscan.h r2scan.h Makefile
