...
want to make a small change to all of the tests, however, since you
must update each test file by hand.

Subtests parameterize the testfile.  You specify the test to run once,
then run it with various different values.  Here is an example:
...
with this file 5 different times, each time specifying a different
value for size.

There's actually no need to specify the L</subtests> header.
This test would work perfectly fine without it (except that the
current config file won't be read, see below for more).
However, you would not receive feedback until all test finished
completely, and you would receive only a pass/fail status.

Specifying the subtests header changes the way tmtest runs
...
tmtest looks for commands beginning with "tmtest".  When found,
these commands are forked in a slightly different way.  All other
commands are executed as normal.

=item *

The subprocess reads the configuration files in the current directory,
instead of the directory containg the subprocess's files.
This means that the subtest will run with the exact same headers
as it would if it were a test in its own file in the current directory.

To make the example above work without the "C<subtests: on>" header,
might need to add "C<-f tmtest.conf>" to cause the subprocess to
read the proper configuration files.

=item *

tmtest automatically interprets the success/fail of the subprocess
and prints each result automatically.  Without this, you'd need to
