...
You can use the TESTFILE and TESTDIR variables to access your testfile
and the directory that contains it.   (echo "$TESTDIR")
MYFILE and MYDIR contain the file and parent dir of the currently
running script.  These will be the same as TESTFILE and TESTDIR
unless the currently running script is a configuration file.
